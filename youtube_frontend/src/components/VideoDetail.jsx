...
import { commentAPI, getVideoByIdAPI, getVideoCommentAPI } from "../utils/fetchFromAPI";
...
const VideoDetail = () => {
  ...
  useEffect(() => {
    ...
    getVideoCommentAPI(id).then(result => {
      setComment(result)
    })
                    ...
                    <div className="card-footer py-3 border-0 text-white" style={{ backgroundColor: '#000' }}>
                      ...
                      <div className="float-end mt-2 pt-1">
                        ...
                        <button type="button" className="btn btn-outline-light btn-sm me-3" onClick={() => {
                          let txtComment = document.querySelector("#textAreaExample").value;
                          let model = {
                            content: txtComment,
                            videoId: id
                          }
